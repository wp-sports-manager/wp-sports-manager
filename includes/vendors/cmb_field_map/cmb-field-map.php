...
class PW_CMB2_Field_Google_Maps {
	...
	public function render_pw_map( $field, $field_escaped_value, $field_object_id, $field_object_type, $field_type_object ) {
		...
		// echo '<input type="text" class="large-text pw-map-search" id="' . $field->args( 'id' ) . '" name="'.$field->args('_name') . '[address]'.'" value="' . isset( $field_escaped_value['address'] ) ? $field_escaped_value['address'] : "" . '"/>';
